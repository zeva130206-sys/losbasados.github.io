...
function initRealtimeFirebase() {
    ...
    // 4. Chat en vivo (Con imágenes ampliables mediante clic)
...
// VISTA PREVIA FLOTANTE DE IMÁGENES (LIGHTBOX)
...
// RENDER DE CATEGORÍAS BAR & SELECT ADMIN
...
// ABRIR MODAL JUEGO & CARGAR SUS COMENTARIOS Y CALIFICACIONES INDEPENDIENTES
function openGameModal(id) {
    ...
    // Estructura principal del juego + sección de comentarios/estrellas
    ...
    // Inicializar listener de Firebase para los comentarios de este juego específico
    initGameComments(id);

    // Eventos del selector de estrellas (si está registrado)
    if (currentUser) {
        selectedRating = 5;
        setupStarSelector();
...
function setupEventListeners() {
    ...
    document.getElementById('formProfile').addEventListener('submit', (e) => {
        ...
        if (currentUser.userId) {
            usersRef.child(currentUser.userId).update({ name: newNick, avatar: newAvatar });
        }
    ...
    // SELECTOR DE EMOJIS RÁPIDO PARA EL CHAT
...
// ACTUALIZAR INTERFAZ DE USUARIO Y ROL MODERADOR
